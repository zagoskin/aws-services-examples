    ...
    public async Task FunctionHandler(S3Event evnt, ILambdaContext context)
    {
        ...
        foreach (var record in eventRecords)
        {
            ...
            try
            {
                ...
                if (response.Metadata["resized"] == true.ToString())
                {
                    context.Logger.LogInformation($"Object {s3Event.Object.Key} was already resized.");
                    continue;
                }

                using var itemStream = await _s3Client.GetObjectStreamAsync(s3Event.Bucket.Name, s3Event.Object.Key, new Dictionary<string, object>());

                using var outStream = new MemoryStream();
                using (var image = await Image.LoadAsync(itemStream))
                {
                    image.Mutate(x => x.Resize(500, 500, KnownResamplers.Lanczos3));
                    var originalName = response.Metadata["originalname"];
                ...
                await _s3Client.PutObjectAsync(new()
                {
                    ...
                    Metadata =
                    {
                        ...
                        ["x-amz-meta-extension"] = response.Metadata["extension"],
                        ["x-amz-meta-originalname"] = response.Metadata["originalname"]
                    },
